...
using UnityEngine;
using GameDevHQ.FileBase.Missle_Launcher.Missile;
    ...
    public class Missile_Launcher : MonoBehaviour
    {
        ...
        IEnumerator FireRocketsRoutine()
        {
            for (int i = 0; i < _misslePositions.Length; i++) //for loop to iterate through each missle position
            {
                ...
                rocket.GetComponent<GameDevHQ.FileBase.Missle_Launcher.Missile.Missile>().AssignMissleRules(_missileType, _target, _launchSpeed, _power, _fuseDelay, _destroyTime); //assign missle properties 
